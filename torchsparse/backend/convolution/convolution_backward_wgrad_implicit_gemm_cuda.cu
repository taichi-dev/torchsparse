...
__global__ void __launch_bounds__(32) conv_backward_cuda_setting1_mode0_f16f16f32(int M_fwd, int K_original, int N, int kernel_volume, int split_k_iters, HALF_TYPE *__restrict__ A, HALF_TYPE *__restrict__ B, int *__restrict__ out_in_map, HALF_TYPE *__restrict__ C)
{
  ...
  float C_warp[8];
  __shared__ HALF_TYPE A_shared[2560];
  __shared__ HALF_TYPE B_shared[2560];
  HALF_TYPE A_shared_warp[8];
  HALF_TYPE B_shared_warp[8];
  HALF_TYPE *cur_C = C + blockIdx_z * kernel_volume * K_original * N;
  for (int i = 0; i < 8; ++i)
  {
    C_warp[0 + i] = 0.0;
  };
  ...
  HALF_TYPE *A_ptr = A + ((threadIdx.y * 256 % 16) % K_tile_padded) + ((threadIdx.x * 8 % 16) % K_tile_padded) + ((blockIdx_y / j_factors1 * 16) % K_tile_padded);
  HALF_TYPE *B_ptr = B + (blockIdx_y % j_factors1) * 16 + (threadIdx.x * 8) % 16;
  ...
  HALF_TYPE *C_ptr = cur_C + (kernel_offset * K_original + cur_C_ic_start) * N + cur_C_oc_start;
  ...
  for (int _i2_0_0 = 0; _i2_0_0 < K_iters - 1; ++_i2_0_0)
  {
    ...
    for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < 4; ++ax0_ax1_fused_0)
    {
      ...
      if (input_idx != -1)
      {
        ...
        // *(uint4 *)(A_ptr_local + input_idx * K_original + ((ax0_ax1_fused_0 * 256 % 16) % K_tile_padded), A_pred_guard);
      ...
      {
        *(uint4 *)(A_shared + (((ax0_ax1_fused_0 * 640) + ((((int)threadIdx.x) >> 1) * 40)) + ((((int)threadIdx.x) & 1) * 8))) = make_uint4(pack_half2(float2half_rn<HALF_TYPE>(0.000000e+00f), float2half_rn<HALF_TYPE>(0.000000e+00f)), pack_half2(float2half_rn<HALF_TYPE>(0.000000e+00f), float2half_rn<HALF_TYPE>(0.000000e+00f)), pack_half2(float2half_rn<HALF_TYPE>(0.000000e+00f), float2half_rn<HALF_TYPE>(0.000000e+00f)), pack_half2(float2half_rn<HALF_TYPE>(0.000000e+00f), float2half_rn<HALF_TYPE>(0.000000e+00f)));
      }
    }
...
#else
  #pragma message("FP16 kernels will not be compiled for SM80-.")
#endif
    }
  }
  ...
  for (int local_id = 0; local_id < 8; ++local_id)
  {
    if constexpr (K_ld_check || N_ld_check)
    {
      if (cur_C_ic_start + ((local_id / 2) % 2) * 8 < K_original && cur_C_oc_start + (local_id % 2) + (local_id / 4) * 8 < N)
        C_ptr[+(((local_id / 2) % 2) * 8) * N + (local_id % 2) + (local_id / 4) * 8] = float2half<HALF_TYPE>(C_warp[0 + local_id]);
    }
    else
    {
      C_ptr[+(((local_id / 2) % 2) * 8) * N + (local_id % 2) + (local_id / 4) * 8] = float2half<HALF_TYPE>(C_warp[0 + local_id]);
...
template<typename HALF_TYPE>
__global__ void __launch_bounds__(64) conv_backward_cuda_setting2_mode0_f16f16f32(int M_fwd, int K_original, int N, int kernel_volume, int split_k_iters, HALF_TYPE *__restrict__ A, HALF_TYPE *__restrict__ B, int *__restrict__ out_in_map, HALF_TYPE *__restrict__ C)
{
  ...
  float C_warp[32];
  __shared__ HALF_TYPE A_shared[2560];
  __shared__ HALF_TYPE B_shared[4608];
  HALF_TYPE A_shared_warp[16];
  HALF_TYPE B_shared_warp[16];
  HALF_TYPE *cur_C = C + blockIdx_z * kernel_volume * N * K_original;
  ...
  HALF_TYPE *A_ptr = A + ((threadIdx.y * 256 % 32) % K_original) + ((threadIdx.x * 8 % 32) % K_original) + ((blockIdx_y / j_factors1 * 32) % K_original);
  HALF_TYPE *B_ptr = B + (blockIdx_y % j_factors1) * 64 + (threadIdx.x * 8) % 64;
  int reorder_offset = threadIdx.y * 256 / 64 + threadIdx.x * 8 / 64;
  HALF_TYPE *C_ptr = cur_C + blockIdx_x / 1 * 108 * N / 16 * 256 + blockIdx_y / j_factors1 * 2 * N / 16 * 256 + (threadIdx.y % 1) * 2 * N / 16 * 256 + (blockIdx_x % 1) * j_factors1 * 64 + (blockIdx_y % j_factors1) * 64 + threadIdx.y / 1 * 32 + (threadIdx.x % 4) * 2 + (threadIdx.x / 4) * N;
  int K_iters = ((M_fwd + 63) / 64 + split_k_iters - 1) / split_k_iters;
  ...
  for (int ax0_0 = 0; ax0_0 < 2; ++ax0_0)
  {
    ...
    HALF_TYPE *C_ptr_local = C_ptr + ax0_0 * N / 16 * 256;
    ...
    for (int ax1_0_2 = 0; ax1_0_2 < 2; ++ax1_0_2)
    {
      for (int local_id = 0; local_id < 8; ++local_id)
      {
        ...
        C_ptr_local[ax1_0_2 * 16 + (((local_id / 2) % 2) * 8) * N + (local_id % 2) + (local_id / 4) * 8] = float2half<HALF_TYPE>(C_warp[((ax0_0 * 16) + (ax1_0_2 * 8)) + local_id]);
      };
    }
  }
}
...
template<typename HALF_TYPE>
void launch_half_kernels(int kernel_size_0, int num_in_channels, int num_out_channels, int kernel_volume, int split_k_iters, HALF_TYPE *in_feats, HALF_TYPE *kernel, int *out_in_map, HALF_TYPE *out_feats) {
  if (num_out_channels % 64 == 0 && num_in_channels % 32 == 0)
  {
    ...
    conv_backward_cuda_setting2_mode0_f16f16f32<HALF_TYPE><<<num_blocks, threads_per_block>>>(
        kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
  }
  else
  {
    ...
    if (num_in_channels % 16 == 0)
    {
      ...
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 16, 16, false, false><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 8 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 16, 16, false, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 4 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 16, 8, false, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 2 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 16, 4, false, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 16, 2, false, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
    }
    else if (num_in_channels % 8 == 0)
    {
      if (num_out_channels % 16 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 16, 16, true, false><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 8 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 16, 16, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 4 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 16, 8, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 2 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 16, 4, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 16, 2, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
    }
    else if (num_in_channels % 4 == 0)
    {
      if (num_out_channels % 16 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 8, 16, true, false><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 8 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 8, 16, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 4 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 8, 8, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 2 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 8, 4, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 8, 2, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
    }
    else if (num_in_channels % 2 == 0)
    {
      if (num_out_channels % 16 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 4, 16, true, false><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 8 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 4, 16, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 4 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 4, 8, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 2 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 4, 4, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 4, 2, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
    }
    else
    {
      if (num_out_channels % 16 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 2, 16, true, false><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 8 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 2, 16, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 4 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 2, 8, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else if (num_out_channels % 2 == 0)
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 2, 4, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
      else
      {
        conv_backward_cuda_setting1_mode0_f16f16f32<HALF_TYPE, 2, 2, true, true><<<num_blocks, threads_per_block>>>(
            kernel_size_0, num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
      }
    }
  }
    ...
    bool allow_tf32, bool allow_fp16, bool allow_bf16)
{
  ...
  if (is_half)
  {
    if (!allow_fp16)
    {
      throw std::runtime_error("FP16 kernels are not supported for implicit GEMM now for SM80-.");
    }
    auto in_feats = reinterpret_cast<half *>(_in_feats.data_ptr<at::Half>());
    auto kernel = reinterpret_cast<half *>(_kernel.data_ptr<at::Half>());
    auto out_feats = reinterpret_cast<half *>(_out_feats.data_ptr<at::Half>());

    launch_half_kernels<half>(_kernel.size(0), num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
  }
  else if (is_bf16)
  {
    if (!allow_bf16)
    ...
    launch_half_kernels<__nv_bfloat16>(_kernel.size(0), num_in_channels, num_out_channels, kernel_volume, split_k_iters, in_feats, kernel, out_in_map, out_feats);
  }
  else if (is_tf)
  {
